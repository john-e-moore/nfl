...
# Select the main div containing the desired data
...
# Directly extracting all player names, over/under lines, and odds from the main div
...
# Iterating through the player names and game props cells in pairs
...
# Convert the DataFrame columns to ASCII, replacing non-ASCII characters
df_ascii = df.applymap(lambda x: x.encode('ascii', 'ignore').decode('ascii') if isinstance(x, str) else x)

# Replace the Unicode minus signs with ASCII hyphen-minuses in the 'over odds' and 'under odds' columns
df_ascii['Over Odds'] = df['Over Odds'].str.replace('−', '-')
df_ascii['Under Odds'] = df['Under Odds'].str.replace('−', '-')

df.to_csv('vegas_player_rec_tds.csv', index=False)
